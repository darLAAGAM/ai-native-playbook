...
A team forking OperAI now has the same onboarding experience that the reference deployment's employees live today: Claude Desktop connected, me.md created, custom instruction applied, Notion checklist in hand — in under 30 minutes per person.
...
```bash
operai-init team-onboard laura --role team --groups cs,retail
```

Which chains:

1. **`operai-init key create laura --role team --groups cs,retail`** — generates `lgm_xxx` key with ACL groups
2. **`operai-init assess laura`** — 10-question interview classifying M-shaped / T-shaped / frontline + 90-day training path
3. **`operai-init team-join --out laura-team-join.sh`** — generates the employee's personalized install script with MCP URL baked in
4. **Email template output** — the founder copy-pastes this to laura, containing:
...
The employee runs the curl, pastes the key, opens Claude Desktop, applies the custom instruction, runs `Run the me-md-interview skill. My name is laura` in Claude Desktop, and in under 30 minutes they are a fully onboarded swarm member.
...
When OperAI improves the onboarding assets, brands pull updates without redeploying the full repo:
...
The logic: the hardest part of AI adoption is not the technology, it's the habits. The onboarding pack is the lever that moves the habits. Keeping it in the base repo maximizes adoption across every brand using OperAI, which in turn maximizes the Pattern Library's cross-company learning (which IS a moat).
...
2. **No automatic me.md interview trigger.** The employee has to type the command. Making Claude Desktop auto-open a session with that command is a Claude Desktop feature request, not an OperAI one.
