...
```bash
curl -fsSL https://useoperai.com/init | bash
```
...
```bash
curl -fsSL 'https://useoperai.com/team-join?brand=<brand>&mcp=mcp.<brand>.com' | bash
```
...
The v0.3 team-join script (generated by `operai-init team-join` or fetched from `useoperai.com/team-join?brand=X&mcp=Y`) prompts the employee for their key and writes a Claude Desktop config like:
